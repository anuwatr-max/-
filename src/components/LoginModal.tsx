...
import { AlertCircle, Loader2, X, ShieldCheck, ExternalLink, Copy, Check, Smartphone, AlertTriangle } from 'lucide-react';
...
export const LoginModal: React.FC<LoginModalProps> = ({
  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-white rounded-3xl p-6 sm:p-7 shadow-2xl border border-slate-200/80 text-slate-900 animate-in zoom-in-95 duration-200 relative max-h-[90vh] overflow-y-auto">
        {/* Close Button */}
        <button
          type="button"
          onClick={onClose}
        ...
        {inApp && (
          <div className="mb-4 p-4 rounded-2xl bg-amber-50 border border-amber-300/80 text-xs text-amber-950 text-left space-y-2.5 shadow-xs animate-in fade-in duration-200">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-amber-800 mt-1 leading-relaxed">
                  Google มีระบบความปลอดภัยที่ไม่อนุญาตให้ล็อกอินในเบราว์เซอร์ของแอป (จะขึ้นข้อผิดพลาด <em>Unable to process request / missing initial state</em>)
                </p>
              </div>
            </div>
            ...
            <div className="pt-1.5 border-t border-amber-200/80 space-y-2">
              <p className="text-[11px] font-semibold text-amber-900">
                👉 วิธีแก้ไข: เปิดระบบใน Google Chrome หรือ Safari
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <button
                  type="button"
                  onClick={handleOpenExternal}
                  className="flex items-center justify-center gap-1.5 px-3 py-2 bg-blue-700 hover:bg-blue-800 text-white font-bold rounded-xl text-xs shadow-xs active:scale-[0.98] transition-all cursor-pointer"
                >
                  <ExternalLink className="h-3.5 w-3.5 shrink-0" />
                  <span>เปิดใน Google Chrome</span>
                </button>
                <button
                  type="button"
                  onClick={handleCopy}
                  className="flex items-center justify-center gap-1.5 px-3 py-2 bg-white hover:bg-amber-100/70 text-amber-900 font-bold border border-amber-300 rounded-xl text-xs active:scale-[0.98] transition-all cursor-pointer"
                >
                  {copied ? (
                    <>
                    ...
                    <>
                      ...
                      <span>คัดลอกลิงก์ไปวาง</span>
                    </>
                  )}
                </button>
              </div>
              <p className="text-[10px] text-amber-700 leading-tight">
                💡 หรือแตะปุ่ม <strong>3 จุด (...)</strong> ที่มุมขวาบน/ล่างของ LINE แล้วเลือก <strong>&quot;เปิดในเบราว์เซอร์อื่น&quot;</strong>
              </p>
            </div>
          </div>
        )}
          ...
          {inApp ? (
            <>
              <button
                type="button"
                onClick={handleOpenExternal}
                className="w-full flex items-center justify-center gap-2.5 px-4 py-3 bg-blue-700 hover:bg-blue-800 text-white font-bold text-xs sm:text-sm rounded-2xl shadow-md shadow-blue-800/20 active:scale-[0.99] transition-all cursor-pointer"
              >
                <ExternalLink className="h-4 w-4 shrink-0" />
                <span>เปิดใน Google Chrome เพื่อเข้าสู่ระบบ</span>
              </button>
              <button
                type="button"
                disabled={isLoading}
                onClick={onSignIn}
                className="w-full flex items-center justify-center gap-2 py-2 text-[11px] text-slate-500 hover:text-slate-800 transition-colors cursor-pointer"
              >
                <span>หรือลองเข้าสู่ระบบในหน้านี้ต่อไป</span>
              </button>
            </>
          ) : (
            <button
