...
export const createSpreadsheet = async (
  ...
  await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/'${SHEET_NAME}'!A1:M${rows.length}?valueInputOption=USER_ENTERED`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${accessToken}`,
      ...
      body: JSON.stringify({
        range: `'${SHEET_NAME}'!A1:M${rows.length}`,
        majorDimension: 'ROWS',
        values: rows,
      }),
    }
...
export const fetchTasksFromSheet = async (
  ...
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/'${SHEET_NAME}'!A2:M`,
    {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
...
export const appendTaskToSheet = async (
  ...
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/'${SHEET_NAME}'!A:M:append?valueInputOption=USER_ENTERED`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
export const updateTaskInSheet = async (
  ...
  const rowData = taskToRow(task);
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/'${SHEET_NAME}'!A${rowNumber}:M${rowNumber}?valueInputOption=USER_ENTERED`,
    {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
export const fullSyncToSheet = async (
  ...
  await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/'${SHEET_NAME}'!A2:M:clear`,
    {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${accessToken}`,
  ...
  if (rows.length > 0) {
    const res = await fetch(
      `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/'${SHEET_NAME}'!A2:M${rows.length + 1}?valueInputOption=USER_ENTERED`,
      {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${accessToken}`,
